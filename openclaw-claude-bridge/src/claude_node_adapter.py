...
INIT_TIMEOUT = 30  # controller 初始化超时（秒）
HEARTBEAT_INTERVAL = 1.5  # 心跳检测间隔（秒）
...
class ClaudeNodeAdapter:
    ...
    def _ensure_controller_alive(self, session_key: str) -> bool:
        ...
        if ctrl is None:
            logger.info(f"[ClaudeNode] _ensure_controller_alive: session={session_key} 不在 _controllers 中")
            return False
        if not ctrl.alive:
            ...
            return False
        logger.info(f"[ClaudeNode] _ensure_controller_alive: session={session_key} alive=True")
        return True
    ...
    def _get_controller(self, session_key: str, system_prompt: str = "", resume: str = "") -> ClaudeController:
        ...
        with self._pool_lock:
            ...
            try:
                ...
                if started:
                    def wait_init():
                        ok = ctrl._wait_for_init(INIT_TIMEOUT)
                        if ok:
                            logger.info(f"[ClaudeNode] session={session_key} 初始化完成（后台）")
                        else:
                            logger.warning(f"[ClaudeNode] session={session_key} 初始化超时（{INIT_TIMEOUT}s）")
    ...
    async def stream_chat(
        ...
        while True:
            try:
                ...
                # 等待 on_message 事件（充当心跳检测）
                try:
                    msg: ClaudeMessage = await asyncio.wait_for(
                        evt_queue.get(), timeout=HEARTBEAT_INTERVAL
                    )
        ...
        if default_key not in self._controllers:
            with self._pool_lock:
                if default_key not in self._controllers:
                    ...
                    ctrl.start(wait_init_timeout=INIT_TIMEOUT)
